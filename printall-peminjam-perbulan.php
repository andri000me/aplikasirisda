...
$pdf->Cell(3, 1, 'Angsuran', 1, 0, 'C');
$pdf->Cell(3, 1, 'Pinjaman', 1, 0, 'C');
$pdf->Cell(3, 1, 'Tgl Pencairan', 1, 0, 'C');
$pdf->Cell(3, 1, 'Jangka Waktu', 1, 1, 'C');

$no = 1;
$query = "SELECT * FROM peminjamperbulan";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $pdf->Cell(3, 2, $row['angsuran'], 1, 0, 'C');
    $pdf->Cell(3, 2, $row['pinjaman'], 1, 0, 'C');
    $pdf->Cell(3, 2, $row['tglpencairan'], 1, 0, 'C');
    $pdf->Cell(3, 2, $row['jangkawaktu'], 1, 0, 'C');
    $pdf->ln(2);
    $no++;
}
